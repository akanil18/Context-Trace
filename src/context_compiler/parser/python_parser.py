    ...
    def parse(self, file_path: Path) -> list[CodeEntity]:
        ...
        for match in cursor.matches(tree.root_node):
            ...
            if "function.def" in captures_dict and "function.name" in captures_dict:
                ...
                if def_node.parent and def_node.parent.type == "block":
                    if def_node.parent.parent and def_node.parent.parent.type == "class_definition":
            ...
            elif "class.def" in captures_dict and "class.name" in captures_dict:
                def_node = captures_dict["class.def"][0]
                ...
                if "class.body" in captures_dict:
                    body_node = captures_dict["class.body"][0]
                    if body_node.named_children:
                        first_stmt = body_node.named_children[0]
                        if first_stmt.type == "expression_statement":
                            expr = first_stmt.named_children[0]
                ...
                signature = ""
                if "class.body" in captures_dict:
                    sig_bytes = source_bytes[def_node.start_byte:captures_dict["class.body"][0].start_byte]
                    signature = sig_bytes.decode("utf-8").strip()
                    if signature.endswith(":"):
                        signature = signature[:-1].strip()
